 ...
 * - `REGEX` creates a phrase from a NovelAI regular-expression string, discarding flags.
 * 
 ...
 */
const { LIT, PRE, POST, OPEN, REGEX } = matching;
 ...
 */
const {
  /** Constructs a phrase expression, with two phrases and one of the operators below. */
  EXP,
  /**
  ...
  ALT,
  /**
   * Ensures that two phrases do or do not appear together somewhere in the search text.
   * `AND` is the default for `subOp`, if it is left unspecified.
   */
  AND, EXCLUDING,
...
// this, but I find it makes using and tweaking keys defining concepts easier.
const keys = {
...
const loreBook = buildEntries({
  ...
  entryConfig: {
    ...
    // These settings should allow more specific entries, those nested in more general entries
  ...
  entries: [
    ...
    },
    {
      ...
      keys: [
        ...
        // matching, because it needs to construct complex, nested regular-expressions.
      ...
      subEntries: [
        {
          ...
          // In this case, these entries will be emitted as `"Character: Taleir - Backstory"`.
          ...
          keys: [],
          // Multiple strings can be provided as an array for `text`.  Each string is given its own
          // entry with the same settings.
          text: [
    ...
    },
    {
      ...
      keys: [
        ...
        // between the two phrases.  The `EXP` helper constructs these expressions in a way that will
        // keep TypeScript from raising false errors.
        //
      ...
      subEntries: [
        {
          ...
          // not "jasco" or that "jeweler" business in the parent.  The `baseKeys` and `baseOp`
          // properties are usually set by the parent and used to pass information about the `keys`
          // and `subOp` of its ancestors to nested entries, but you can completely override this
      ...
      subEntries: [
        {
          ...
          // In this case, we will not reduce `contextConfig.budgetPriority`, as these qualities are
          // more likely to affect the course of the story, especially for the main character.
          keys: [],
          text: [
        ...
        // This entry will demonstrate a very complicated composition and some advanced features.
        // The vulpine have two deities, Deynai and Trunai.  We want to be able to provide this
        // information when foxes are mentioned in a religious context or the name of one of their
        // gods is mentioned.
        {
          name: "Religion",
          // The parent specified a `subOp` of `WITH`, but I want this entry, and only this entry, to
          // use `AND` instead.  Note that the `baseOp` will be used as the `subOp` for nested entries
          // unless you tell it otherwise by setting this entry's `subOp` property.
          baseOp: AND,
          keys: keys.religion,
          // We're skipping `text` this time, only using this to configure the nested entries.
          subEntries: [
            {
              name: "Deynai",
              // We're going to build quite a complex matcher here.  `baseKeys` accepts a function
              // which can receive the parent's keys and manipulate them however you need.
              baseKeys: (baseKeys) => [
                // In this case, we only want to match the `keys.religion` stuff when it does not
                // appear with one of the names of the vulpine deities.  This allows these entries
                // to match a generic phrase like, "a vulpine walks out of a church," and inject
                // information about their religion into context.
                ...baseKeys.map((k) => EXP(k, EXCLUDING, ALT(...keys.deynai, ...keys.trunai))),
                // But we have two deities.  If this specific deity is mentioned, the context will
                // be focused on that deity instead, helping to reduce the likelihood of a "lore dump"
                // that dominates the context with too much information about the vulpine religion.
                ...keys.deynai
              ...
              // We're actually using `baseKeys` as the primary source.  The `keys` property is
              // specified as required in the types, though, to help ensure you don't accidentally
              // forget to provide them (or explicitly NOT provide them, in this case).
              keys: [],
              text: [
